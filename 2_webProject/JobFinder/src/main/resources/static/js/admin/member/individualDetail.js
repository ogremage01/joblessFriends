...
deleteBtn.addEventListener("click", function(e) {
    ...
    // 사용자에게 확인 요청
    const confirmed = confirm("강제탈퇴를 진행합니까?\n\n※ 주의: 해당 회원의 모든 이력서, 지원 이력, 북마크 등이 함께 삭제됩니다.");
    if (confirmed) {
        // 로딩 표시
        ...
        .catch(error => {
            ...
            } else if (error.message.includes("이력서")) {
                errorMessage = "회원의 이력서 데이터 삭제 중 오류가 발생했습니다.";
            } else if (error.message.includes("지원")) {
                errorMessage = "회원의 지원 이력 삭제 중 오류가 발생했습니다.";
            } else {
                errorMessage = "회원 탈퇴 처리 중 오류가 발생했습니다: " + error.message;
            }
